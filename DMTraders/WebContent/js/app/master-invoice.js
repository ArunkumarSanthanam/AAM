...
function loadInvoiceTable(){
	...
	var columnArrayLog = [ {"title":"<input name='select_all' value='1' id='example-select-all' type='checkbox' />", "class" : "td2", "width" : "5%"},
	                       ...
	                       {"title" : "Stock Quantity","data" : "pickingQuantity","class" : "td2","width" : "5%"},
	                       {"title" : "Bill Quantity","data" : "billQuantity","class" : "td2","width" : "20%"},
...
console.log("Inside"+saleOrderNumber);
		$.ajax({
	 		url : "invoises?saleOrderNumber="+saleOrderNumber,
	 		type : 'POST',
	 			 ...
	 			 $('#saleOrderTable tbody').off();
	 				$('#saleOrderTable tbody').on('click', 'td', function() {
	 						var colIndex = $(this).index();
	 					 	...
	 					 	if(colIndex != 0) {
	 					 		var invoiceId = myTable.row( this ).data().invoiceId;
	 					 		console.log(myTable.row( this ).data());
	 					 		$("#invoiceIdTemp").val(invoiceId);
	 					 		$.ajax({
	 						 		url : "getinvoicepopup?invoiceId="+invoiceId,
	 						 		type : 'POST',
	 						 		contentType: "application/json;charset=utf-8",
	 						        dataType: "json",
	 						 		async : true,		
	 						 		success : function(result) {
	 						 			//getSaleOrderResponse(custId);
	 						 			$("#productName option[value='"+result.productName+"']").attr("selected", "selected").trigger("change");
	 									$("#brandName option[value='"+result.brandName+"']").attr("selected", "selected").trigger("change");
	 									$("#orderQuantity").val(result.orderQuantity);
	 									$("#billQuantity").val(result.billQuantity);
	 									$("#pickingQuantity").val(result.pickingQuantity);
	 									$("#sellingPrice").val(result.sellingPrice);
	 									$("#addSaleProduct").trigger("click");
	 						 		}
	 					 		});
